...
SystemKey = "e0d083ab0b8abe99fd86c0bef802"
SystemSecret = "E0D083AB0BDAEAFE8B8F8CB4E315"
SystemURL="https://presales.clearblade.com"
cbUser="mqtt@clearblade.com"
cbPass="clearblade"
LOGLEVEL="INFO"
TOPIC="device/ThunderBoard/"
RED=255
GREEN=0
BLUE=255
LIGHT=255
    ...
    for (adtype, desc, value) in dev.getScanData():
        if desc == 'Complete Local Name':
            if 'Thunder Sense #' in value:
                logging.info("%s :: %s :: %s ",  dev.addr, value, dev.rssi)
                try:
                    tbdata=dict()
                    ...
                    for k in characteristics:
                        ...
                        if k.uuid == '2a6e':
                            ...
                            s=chr(LIGHT)+chr(RED*BRIGHTNESS//100)+chr(GREEN*BRIGHTNESS//100)+chr(BLUE*BRIGHTNESS//100)
                            k.write(s, True)                           
                    logging.debug(json.dumps(tbdata))
                    mqtt.publish(TOPIC + dev.addr, json.dumps(tbdata))
